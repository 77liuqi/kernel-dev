				       ...
				       u64 *cmds, int n, bool sync)
{
	...
	struct arm_smmu_ll_queue llq = {
		...
		.max_n_shift = cmdq->q.llq.max_n_shift,
		.prod = (n + sync) + (2 << cmdq->q.llq.max_n_shift),
	};
	...
	llq.prod &= prod_mask;
	owner = !(llq.prod & prod_mask);
	head.prod = queue_inc_prod_n(&llq, n + sync);

	if (atomic64_read(&tries) < 20)
		pr_err("cmdlist0 cpu%d head=0x%x 0x%x llq=0x%x 0x%x n=%d sync=%d tries=0x%llx\n", cpu, head.prod, head.cons, llq.prod, llq.cons, n, sync, atomic64_read(&tries));
	...
	if (atomic64_read(&tries) < 20)
		pr_err("cmdlist1 cpu%d head=0x%x 0x%x llq=0x%x 0x%x n=%d sync=%d owner_val=0x%x owner=%d\n", cpu, head.prod, head.cons, llq.prod, llq.cons, n, sync, owner_val, owner);
	...
	if (owner) {
		...
		prod = atomic_fetch_andnot_relaxed(~prod_mask,
						       &cmdq->q.llq.atomic.prod);
		prod = Q_WRP(&llq, prod) | Q_IDX(&llq, prod);
		
		if (atomic64_read(&tries) < 20)
			pr_err("cmdlist2 cpu%d head=0x%x 0x%x llq=0x%x 0x%x n=%d sync=%d llq.prod=0x%x prod=0x%x\n", cpu, head.prod, head.cons, llq.prod, llq.cons, n, sync, llq.prod, prod);
	...
	if (atomic64_read(&tries) < 20)
		pr_err("cmdlist3 cpu%d head=0x%x 0x%x llq=0x%x 0x%x n=%d sync=%d prod=0x%x\n", cpu, head.prod, head.cons, llq.prod, llq.cons, n, sync, prod);
	...
	if (sync) {
		...
		if (ret) {
			dev_err_ratelimited(smmu->dev,
					    "CMD_SYNC timeout at 0x%08x [hwprod 0x%08x, hwcons 0x%08x] owner=%d n=%d sync=%d cpu%d head=0x%x 0x%x\n",
					    llq.prod,
