...
#define Q_PROD(llq, p)		(Q_IDX(llq, p) | Q_WRP(llq, p))
...
static __maybe_unused bool queue_has_space(struct arm_smmu_ll_queue *q, u32 n, u32 *myspace)
{
	u32 space, prod, cons;
	u32 diff = -1;
	...
	if (Q_WRP(q, q->prod.prod) == Q_WRP(q, q->cons))
...
//	if (space < n)
//		pr_err_once("%s no space prod=0x%x cons=0x%x diff=0x%x 1 << q->max_n_shift=0x%x n=%d space=%d q->prod.prod=0x%x q->cons=0x%x\n",
//		__func__, prod, cons, diff, 1 << q->max_n_shift, n, space, q->prod.prod, q->cons);
...
{
	bool first = Q_IDX(q, q->prod.prod) == Q_IDX(q, q->cons) &&
	       Q_WRP(q, q->prod.prod) != Q_WRP(q, q->cons);
	u32 my_space;

	bool second = !queue_has_space(q, 1, NULL);


	if (first != second)
		pr_err_once("%s first=%d second=%d q->prod.prod=0x%x q->cons=0x%x my_space=%d cpu%d\n",
		__func__, first, second, q->prod.prod, q->cons, my_space, smp_processor_id());
...
static bool queue_empty(struct arm_smmu_ll_queue *q)
{
	...
	bool second = (q->cons == q->prod.prod);

	if (first != second)
		pr_err_once("%s first=%d second=%d q->prod.prod=0x%x q->cons=0x%x\n",
		__func__, first, second, q->prod.prod, q->cons);
...
{
	bool first = ((Q_WRP(q, q->cons) == Q_WRP(q, prod)) &&
					       ...
					       u32 sprod, u32 eprod, bool set)
{
	...
	while (llq.prod.prod != eprod) {
		unsigned long mask;
		atomic_long_t *ptr;
		u32 limit = BITS_PER_LONG;

		swidx = BIT_WORD(Q_IDX(&llq, llq.prod.prod));
		sbidx = Q_IDX(&llq, llq.prod.prod) % BITS_PER_LONG;
		...
		if (ktime_after(ktime_get(), start+ms_to_ktime(1000)))
			pr_err("%s cpu%d sprod=0x%x eprod=0x%x set=%d\n", __func__, smp_processor_id(), sprod, eprod, set);
...
u32 arm_smmu_get_cons(struct arm_smmu_ll_queue *llq, struct arm_smmu_cmdq *cmdq)
{
	...
	if ((Q_WRP(llq, read_value) == 0) && (Q_WRP(llq, inti_sw_cons_main))) { // sw cons was 1, and now cycled, so add
		...
		read_value |= wrpplus;
		special = true;
		pr_err_once("%s2 cpu%d cmdq->q.llq.cons=0x%x read_value=0x%x wrpplus=0x%x inti_sw_cons=0x%x orig_hw=0x%x inti_sw_cons_main=0x%x\n", __func__, smp_processor_id(), llq->cons, read_value, wrpplus, inti_sw_cons, orig_hw, inti_sw_cons_main);

	pr_err_once("%s2.1 cpu%d cmdq->q.llq.prod.prod=0x%x cmdq->q.llq.cons=0x%x llq->cons=0x%x read_value=0x%x orig_hw=0x%x inti_sw_cons=0x%x special=%d wrpplus=0x%x orig_hw_prod=0x%x cmdq_prod=0x%x inti_sw_cons_main=0x%x\n", 
				__func__, smp_processor_id(), cmdq->q.llq.prod.prod, cmdq->q.llq.cons, llq->cons, read_value, orig_hw, inti_sw_cons, special, wrpplus, orig_hw_prod, cmdq_prod, inti_sw_cons_main);

		
	} else if (special_wrap) {
		
		pr_err("%s3 cpu%d cmdq->q.llq.cons=0x%x llq->cons=0x%x read_value=0x%x orig_hw=0x%x inti_sw_cons=0x%x inti_sw_cons_main=0x%x\n", __func__,
		smp_processor_id(), cmdq->q.llq.cons, llq->cons, read_value, orig_hw, inti_sw_cons, inti_sw_cons_main);
		wrpplus = inti_sw_cons_main >> llq->max_n_shift;
		wrpplus += 2;
		wrpplus = wrpplus << llq->max_n_shift;
		read_value = Q_PROD(llq, read_value);
		...
		pr_err("%s3.1 cpu%d cmdq->q.llq.cons=0x%x llq->cons=0x%x read_value=0x%x orig_hw=0x%x inti_sw_cons=0x%x wrpplus=0x%x llq prod=0x%x orig_hw_prod=0x%x inti_sw_cons_main=0x%x\n", __func__,
	smp_processor_id(), cmdq->q.llq.cons, llq->cons, read_value, orig_hw, inti_sw_cons, wrpplus, llq->prod.prod, orig_hw_prod, inti_sw_cons_main);
	} else {
		u32 print = false;
	...
	//		print = true;
		...
		if (print)
			pr_err_once("%s4 cpu%d cmdq->q.llq.cons=0x%x llq->cons=0x%x read_value=0x%x orig_hw=0x%x inti_sw_cons=0x%x wrpplus=0x%x inti_sw_cons_main=0x%x\n", __func__,
		smp_processor_id(), cmdq->q.llq.cons, llq->cons, read_value, orig_hw, inti_sw_cons, wrpplus, inti_sw_cons_main);
	}
	...
	if (read_value < inti_sw_cons) {
		if (1)
			panic("ddd cpu%d cmdq->q.llq.prod.prod=0x%x cmdq->q.llq.cons=0x%x llq->cons=0x%x read_value=0x%x orig_hw=0x%x inti_sw_cons=0x%x special=%d wrpplus=0x%x orig_hw_prod=0x%x special_wrap=%d max_n_shift=%d inti_sw_cons_main=0x%x\n", 
					  ...
					  struct arm_smmu_ll_queue *llq)
{
	...
	struct arm_smmu_queue_poll qp;
	struct arm_smmu_cmdq *cmdq = &smmu->cmdq;
	u32 *cmd = (u32 *)(Q_ENT(&cmdq->q, llq->prod.prod));

//	if (llq->prod.prod != Q_IDX(llq, llq->prod.prod))
//		pr_err_once("%s wrong llq->prod.prod=0x%x Q_IDX(llq, llq->prod.prod)=0x%x\n", __func__, llq->prod.prod, Q_IDX(llq, llq->prod.prod));
	...
	llq->cons = ret ? llq->prod.prod : queue_inc_prod_n(llq, 1);
	return ret;
}
...
({									\
	...
	for (;;) {							\
		...
		if (ktime_after(ktime_get(), start+ms_to_ktime(1000)))\
			pr_err_once("%sx cpu%d VAL=0x%x llq.prod.prod=0x%x cpu%d head.prod.prod=0x%x initial_prod=0x%x initial_val=0x%llx\n", __func__, cpu, VAL, llq.prod.prod, smp_processor_id(), head.prod.prod, initial_prod, initial_val);\
	}								\
	(typeof(*ptr))VAL;						\
})
...
{
	u64 cmd_sync[CMDQ_ENT_DWORDS], prod64;
	u32 prod;
	unsigned long flags;
	bool owner;
	struct arm_smmu_cmdq *cmdq = &smmu->cmdq;
...
//		cpu, llq.prod.prod, llq.prod.owner, llq.cons, prod64, n, sync);
	initial_val = llq.prod.val = atomic64_fetch_add(prod64, &cmdq->q.llq.prod.atomic_val.val);

	initial_prod = llq.prod.prod;
...
//	__func__, cpu, llq.prod.prod, llq.prod.owner, llq.cons, prod64, owner, head.prod.prod, llq.prod.prod);
	...
	//	__func__, cpu, llq.prod.prod, llq.prod.owner, llq.cons, prod64, owner, head.prod.prod, prod);
...
//	pr_err("%s4 cpu%d llq.prod=[0x%x 0x%x] head.prod.prod=0x%x owner=%d\n",
	...
	//	__func__, cpu, llq.prod.prod, llq.prod.owner, llq.cons, prod64, owner, head.prod.prod, msk, cmdq->q.llq.prod.prod, cmdq->q.llq.prod.owner);
		...
		prod64 = atomic64_fetch_andnot_relaxed(mask.prod.val,
	...
	//	if (prod > 0xffc0)
	//		pr_err("%s2 cpu%d prod=0x%x prod64=0x%llx llq.prod.prod=0x%x llq.cons=0x%x head.prod.prod=0x%x\n", __func__, cpu, prod, prod64, llq.prod.prod, llq.cons, head.prod.prod);
...
//		if (prod > 0xffc0)
//			pr_err("%s2.1 cpu%d prod=0x%x prod64=0x%llx llq.prod.prod=0x%x llq.cons=0x%x head.prod.prod=0x%x\n", __func__, cpu, prod, prod64, llq.prod.prod, llq.cons, head.prod.prod);

//		pr_err("%s6.1 cpu%d prod=0x%x\n", __func__, cpu, prod);


		if (prod != Q_PROD(&llq, prod))
			pr_err_once("%s6.2 cpu%d owner setting hw prod=0x%x Q_PROD(prod)=0x%x prod64=0x%llx llq.prod.prod=0x%x initial_prod=0x%x\n",
				__func__, cpu, prod, Q_PROD(&llq, prod), prod64, llq.prod.prod, initial_prod);

		if (llq.prod.prod > 0x25000)
			pr_err_once("%s6.3 cpu%d owner setting hw prod=0x%x Q_PROD(prod)=0x%x prod64=0x%llx llq.prod.prod=0x%x initial_prod=0x%x\n",
				__func__, cpu, prod, Q_PROD(&llq, prod), prod64, llq.prod.prod, initial_prod);
	...
	//	if (prod > 0xffc0)
	//		pr_err("%s3 cpu%d prod=0x%x llq.prod.prod=0x%x llq.cons=0x%x head.prod.prod=0x%x\n", __func__, cpu, prod, llq.prod.prod, llq.cons, head.prod.prod);
...
//	__func__, cpu, llq.prod.prod, llq.prod.owner, llq.cons, prod64, owner, head.prod.prod);
...
static irqreturn_t arm_smmu_evtq_thread(int irq, void *dev)
{
	...
	llq->cons = Q_OVF(llq->prod.prod) | Q_WRP(llq, llq->cons) |
		    Q_IDX(llq, llq->cons);
	return IRQ_HANDLED;
}
...
static int arm_smmu_device_reset(struct arm_smmu_device *smmu, bool bypass)
{
	...
	writel_relaxed(smmu->cmdq.q.llq.prod.prod, smmu->base + ARM_SMMU_CMDQ_PROD);
	...
	writel_relaxed(smmu->evtq.q.llq.prod.prod,
	...
	if (smmu->features & ARM_SMMU_FEAT_PRI) {
		...
		writel_relaxed(smmu->priq.q.llq.prod.prod,
