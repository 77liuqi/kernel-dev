...
atomic64_t iova_allocs_has_pfn_useful;
atomic64_t iova_allocs_rcache_alloc_fail;
atomic64_t iova_allocs_rcache_alloc_fail_flush;
...
unsigned long print_cpu_iova(int cpu, struct iova_domain *iovad, bool  print_cpus)
{
	...
	for (i = 0; i < IOVA_RANGE_CACHE_MAX_SIZE; ++i) {
		...
		if (prev)
			total += prev->size;
		if (print_cpus) {
			sprintf(string + strlen(string), "i=%d l=%lu p=%lu, ", i, 
...
retry:
	...
	if (!new_iova) {
		unsigned int cpu;
		atomic64_inc(&iova_allocs_rcache_alloc_fail);

		if (!flush_rcache)
			return 0;
		
		atomic64_inc(&iova_allocs_rcache_alloc_fail_flush);
				     ...
				     size_t real_size)
{
	...
	if (size > iovad->biggest_size) {
		iovad->biggest_size = size;
		changed = 1;
	}

	if (limit_pfn > iovad->biggest_limit_pfn) {
		iovad->biggest_limit_pfn = limit_pfn;
		changed = 1;
	}

	if (changed)
		pr_err("%s biggest_size=%lu biggest_limit_pfn=%lu log_size=%d too_big=%d real_size=%zu\n",
			__func__, iovad->biggest_size, iovad->biggest_limit_pfn, log_size,
