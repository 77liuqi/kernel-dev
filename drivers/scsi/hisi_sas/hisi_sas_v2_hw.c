...
static bool auto_affine_interrupts = true;
...
static void setup_reply_map_v2_hw(struct hisi_hba *hisi_hba)
{
	...
	for (queue = 0; queue < hisi_hba->queue_count; queue++) {
		...
		struct irq_affinity_desc *mask = &hisi_hba->affinity_masks[queue];
		struct cpumask *cpumask = &mask->mask;
...
static int interrupt_init_v2_hw(struct hisi_hba *hisi_hba)
{
	...
	for (queue_no = 0; queue_no < hisi_hba->queue_count; queue_no++) {
		...
		rc = devm_request_threaded_irq(dev, cq->irq_no, cq_interrupt_v2_hw,
				cq_thread_v2_hw, irqflags, DRV_NAME " cq", cq);
		if (rc) {
