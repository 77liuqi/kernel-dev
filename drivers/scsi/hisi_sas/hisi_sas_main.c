				...
				void *funcdata)
{
	...
	int phy_no = sas_phy->id;
	bool is_wait_for_phyup = true;
	u8 status = phy->phy_attached;
	int ret = 0;

	phy->reset_completion = &reset_completion;
	switch (func) {
	...
	if (status && is_wait_for_phyup) {

		if (!wait_for_completion_timeout(&reset_completion, 2 * HZ)) {
			dev_warn(dev, "phy%d of func%d wait phyup timeout\n",
				 phy_no, func);
			if (phy->in_reset)
				ret = -ETIMEDOUT;
		}
...
static int hisi_sas_I_T_nexus_reset(struct domain_device *device)
{
	...
	if (rc == TMF_RESP_FUNC_COMPLETE && dev_is_sata(device)) {
		...
		case -EIO:
			...
			if (!rc) {
				local_phy->enabled = 0;
				dev_err(dev, "Disabling local phy of ATA disk %016llx due to softreset fail (%d)\n",
