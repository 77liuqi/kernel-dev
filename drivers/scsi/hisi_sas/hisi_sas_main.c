...
static void hisi_sas_tmf_abort_handler(struct sas_task *task)
{
	...
	pr_err("TMF (%d) task timeout and not done %016llx\n", tmf->tmf,
	       SAS_ADDR(device->sas_addr));
...
static int hisi_sas_execute_ata_cmd(struct domain_device *device, u8 *fis,
					int force_phy_id)
{
	struct sas_tmf_task tmf = {
		.hisi_sas_abort_handler = hisi_sas_tmf_abort_handler,
	};

	return sas_execute_ata_cmd(device, fis, &tmf, force_phy_id);
}
...
static int hisi_sas_softreset_ata_disk(struct domain_device *device)
{
	...
	ata_for_each_link(link, ap, EDGE) {
		...
		rc = hisi_sas_execute_ata_cmd(device, fis, -1);
		if (rc != TMF_RESP_FUNC_COMPLETE)
			break;
	}
					     ...
					     struct domain_device *device)
{
	...
	list_for_each_entry(sas_phy, &sas_port->phy_list, port_phy_el) {
		...
		ata_for_each_link(link, ap, EDGE) {
			...
			hisi_sas_fill_ata_reset_cmd(link->device, 1, pmp, fis);
			rc = hisi_sas_execute_ata_cmd(device, fis, sas_phy->id);
			if (rc != TMF_RESP_FUNC_COMPLETE) {
