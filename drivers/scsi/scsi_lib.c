		 ...
		 int *resid)
{
	...
	if (cmd[0] == INQUIRY)
		dev_err(sdev->request_queue->dev, "%s INQUIRY rq_flags & RQF_PM=0x%x pm_only=%d q rpm status=%d q->mq_freeze_depth=%d sdev->request_queue->dev=%pS\n",
		__func__, rq_flags & RQF_PM, blk_queue_pm_only(sdev->request_queue), queue_rpm_status(sdev->request_queue),
		sdev->request_queue->mq_freeze_depth, sdev->request_queue->dev);
