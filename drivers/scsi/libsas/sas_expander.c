				...
				int phy_id, u8 *sas_addr)
{
	...
	pr_err("ex %016llx phy%d broadcast flutter%s\n",
		 SAS_ADDR(dev->sas_addr), phy_id, action);

	if (linkrate != phy->linkrate) {
		pr_err("ex %016llx phy%d linkrate changed from %d to %d\n",
			 SAS_ADDR(dev->sas_addr), phy_id,
			 linkrate, phy->linkrate);
		return false;
	}
	...
	if (SAS_ADDR(phy->attached_sas_addr) != SAS_ADDR(sas_addr) ||
		...
		pr_err("phy address(%016llx) abnormal, origin:%016llx\n",
			 SAS_ADDR(phy->attached_sas_addr),
			 SAS_ADDR(sas_addr));
		return false;
	}
			      ...
			      bool *retry)
{
	...
	pr_err("ex %016llx unregistering phy%02d\n",
		 SAS_ADDR(dev->sas_addr), phy_id);
			   ...
			   bool *retry)
{
	...
	for (i = 0; i < ex->num_phys; i++) {
		...
		if (SAS_ADDR(phy->attached_sas_addr) ==
			...
			pr_err("phy%02d part of wide port with phy%02d\n",
				 phy_id, i);
			last = false;
			break;
		}
	...
	pr_err("ex %016llx phy%d discover returned 0x%x\n",
		 SAS_ADDR(dev->sas_addr), phy_id, res);
}
				 ...
				 int nr, bool *retry)
{
	...
	for (i = 0; i < nr; i++) {
		pr_err("ex %016llx phy%d originated BROADCAST(CHANGE)\n",
			    ...
			    int nr)
{
	...
	for (i = 0; i < nr; i++) {
		...
		pr_err("ex %016llx phy%d register returned 0x%x\n",
